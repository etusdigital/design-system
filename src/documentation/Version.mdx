...
This is the React migration release. The design system has been fully migrated from Vue to React.
...
All components have been migrated from Vue to React. Key changes:

- `v-model` bindings replaced with `value` + `onChange` props
- Vue slots replaced with React `children` and named props
- Vue event syntax (`@eventName`) replaced with React event props (`onEventName`)
- Vue-specific directives (`v-if`, `v-for`, `v-show`) replaced with JSX conditional rendering and `.map()`
- `inject('toast')` replaced with `useToast()` hook
- `inject('confirm')` replaced with `useConfirm()` hook
- `createApp` / `app.use(DesignSystem)` replaced with `<DesignSystemProvider>`
...
## Stepper
  ...
  - added getObject prop for returning full option objects
...
## TagInput
  - the prop inputMask was removed and replaced with mask
  - the prop tagList was removed and replaced with value/onChange
