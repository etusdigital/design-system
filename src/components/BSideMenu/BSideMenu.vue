...
onBeforeMount(() => {
  const item = getPaths(props.items, props.parentPath).find((item) => checkPath(item.path));
  if (item) changeModel(item.value);
});

function getPaths(items: ItemType[], parentPath: string = ''): { path: string, value: any }[] {
...
<style scoped>
.b-side-menu {
  @apply flex flex-col gap-sm w-fit py-lg px-sm bg-default border-r-xxs border-r-neutral-default;
