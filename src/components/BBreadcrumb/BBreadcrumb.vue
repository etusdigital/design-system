...
async function showMoreItems(event: any, index: number) {
  ...
  const card = document.querySelector(`.more-options[index="${index}"]`) as HTMLElement;
  if (!card) return;
...
<template>
  <div class="b-breadcrumb">
    <template v-for="(item, index) in parsedItems" :key="item">
      <div v-if="isObject(item) && item.icon == 'more_horiz'" @click="showMoreItems($event, index)">
        <BIcon name="more_horiz" class="cursor-pointer" />
        <Teleport to="body">
          <Transition name="fade">
            <b-card v-if="expanded[index]" class="more-options" :index="index">
