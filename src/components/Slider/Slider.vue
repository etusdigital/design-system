...
import { blendColors, getPosition } from "../../utils";
...
const isDraggingSlider = ref(props.isRange ? [false, false] : [false]);
...
watch(
  ...
  () => {
    ...
    if (
      !model.value.length ||
      value.find(
        (value, index) => model.value && value != model.value[index]
      ) != undefined
    )
      setModel(value);
  }
...
watch(
  () => props.isRange,
  () => {
    isDraggingSlider.value = props.isRange ? [false, false] : [false];
  }
);

function setModel(value: number[]) {
  model.value = value;
  emit("update:modelValue", props.isRange ? value : value[0]);
}

function updateModel(value: number | number[]) {
  if (!Array.isArray(value)) model.value = [value];
  else if (value) model.value = value;
  else model.value = [];
}
...
function getPercentage() {
  ...
  model.value.forEach((value: any, index: number) => {
