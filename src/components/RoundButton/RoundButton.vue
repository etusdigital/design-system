...
<template>
  ...
  >
    <div class="content">
      <Icon :name="computedIcon" class="icon" />
      <span class="label" v-if="labelValue || text">{{ labelValue || text }}</span>
    </div>
  </button>
</template>
...
.round-button.hovered .label,
.round-button.always-open .label {
  @apply opacity-100 scale-100 max-w-[200px] mr-base;
}

.round-button.always-open {
