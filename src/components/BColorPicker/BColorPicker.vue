<script setup lang="ts">
	...
	function updateOpacitySlider(event: any) {
		if (isDraggingOpacitySlider.value && cursorOpacitySlider.value) {
			...
			const clampedLeft = getCursorPosition(
				event,
	...
	function updateColorSlider(event: MouseEvent) {
		if (isDraggingColorSlider.value && cursorColorSlider.value) {
			...
			const sliderOpacityDiv = cursorOpacitySlider.value?.closest(
				".slider"
			) as HTMLDivElement;
	...
	function updateColorArea(event: MouseEvent) {
		if (isDraggingColorArea.value && cursorColorArea.value) {
			...
			const clamped = getCursorPosition(
				event,
			...
			circleBackground.value = `rgba(${pixel[0]}, ${pixel[1]}, ${pixel[2]}, ${
				pixel[3] / 255
			})`;
	...
	function calculateCursor() {
		...
		const rgba = hsvaToRgba(
			hsva.h,
			Number(hsva.s.toString().replace("%", "")),
			Number(hsva.v.toString().replace("%", "")),
			hsva.a
		);
	...
	function getColor(color: any) {
		if (colorType.value == 0) {
			...
			return `${hsla.h.toFixed(0)}, ${hsla.s}, ${hsla.l}, ${
				hsla.a < 1 ? hsla.a.toFixed(2) : hsla.a
			}`;
		} else if (colorType.value == 2) {
			const hwb = rgbaToHwb(color.r, color.g, color.b, color.a);
			return `${hwb.h} ${hwb.w} ${hwb.b} / ${
				hwb.a < 1 ? hwb.a.toFixed(2) : hwb.a
			}`;
		} else if (colorType.value == 3) {
			const hsva = rgbaToHsva(color.r, color.g, color.b, color.a);
			return `${hsva.h}, ${hsva.s}, ${hsva.v}, ${
				hsva.a < 1 ? hsva.a.toFixed(2) : hsva.a
			}`;
		}
		return `${color.r}, ${color.g}, ${color.b}, ${
			color.a < 1 ? color.a.toFixed(2) : color.a
		}`;
	}
	...
	function findColorPosition(color: any): any {
		...
		const formatedColor = {
			h: color.h,
			s:
				typeof color.s == "string" ? Number(color.s.replace("%", "")) : color.s,
			v:
				typeof color.v == "string" ? Number(color.v.replace("%", "")) : color.v,
			a: color.a,
		};
		const positionX = Math.max(
			-5,
			Math.min(
				(formatedColor.s / 100) * (canvas.clientWidth - 5),
		...
		const positionY = Math.max(
			0,
			Math.min(
				(1 - formatedColor.v / 100) * canvas.clientHeight,
				canvas.clientHeight
			)
		);
