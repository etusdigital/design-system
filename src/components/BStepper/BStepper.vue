		...
		<div
			...
			<div
				...
				<button
					...
					:data-after-content="item.label ? item.label : item"
					:class="{
						'active-button': index === step,
						'past-button': index <= biggerStepSelected,
					...
					@click="changeActiveStep(item, index)">
					<BIcon :name="item.icon ? item.icon : 'image'" />
				</button>
			</div>
			<span
				class="right-line"
...
<style scoped>
	...
	.step-button {
		@apply flex items-center justify-center border-xxs h-[2.2em] w-[10em] border-neutral-border-default text-neutral-interaction-default pl-base ml-xs font-bold uppercase text-sm text-start relative bg-default;
	...
	.step-button:not(.fisrt-button)::before {
		@apply z-2 absolute w-[20px] h-full -left-px top-1/2 translate-y-[-50%] bg-default;
		content: "";
		background: v-bind(background);
	...
	.large .step-button-circle .b-icon {
		@apply text-3xl;
	}
