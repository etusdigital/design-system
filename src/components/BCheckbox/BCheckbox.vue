...
<style scoped>
	...
	.b-checkbox:hover {
		.content {
			border-color: var(--color-primary-interaction-hover);
		}
	...
	.b-checkbox.disabled {
		...
		.content {
			@apply bg-neutral-surface-default text-primary-surface-pressed;
	...
	.content {
		@apply bg-neutral-surface-default text-neutral-foreground-negative inline-block h-base w-base border rounded-xs transition-all duration-150 ease-out select-none;
		border-color: var(--color-primary-border-default);
	}

	.content.active {
		@apply bg-primary-interaction-selected;
		border-color: var(--color-primary-interaction-selected);
	}
